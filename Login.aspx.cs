...
public partial class Login : System.Web.UI.Page
{
    ...
    private DataTable getDetails(string userid,string Password)
    {
        string qry = "select * from participantcontributiondetails where cnic=@cnic and password=@password";
    ...
    protected void btnLogin_Click(object sender, EventArgs e)
    {
        ...
        DataTable getUser = getDetails(txtUsername.Text, txtPassword.Text);
        if (getUser.Rows.Count > 0)
        {
            if (getUser.Rows[0]["VPS_ACCOUNT_TYPE"].ToString() == "PF")
            {
            ...
            {
                string folioWithDashes = getUser.Rows[0]["cnic"].ToString();
                Session["FolioNo"] = string.Format("{0}-{1}-{2}", folioWithDashes.Substring(0, 5), folioWithDashes.Substring(5, 7), folioWithDashes.Substring(12));
                Session["TITLE"] = getUser.Rows[0]["participant_name"];
                Session["ID"] = getUser.Rows[0]["Participant_ID"];
                Session["VPS_CORPORATE_ID"] = getUser.Rows[0]["VPS_CORPORATE_ID"];
            }
            if (txtPassword.Text.ToString() != "P@ss1234")
            {
